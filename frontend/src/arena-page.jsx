...
const BATTLE_TRAINING_RANKS = [
  ...
  ['grandmaster', 'Grandmaster', 'Hardest exam-style sets'],
]
...
function ArenaGameLobby({ exam, setExam, paused, name, rank, rankProgress, avatar, openCustomizer, mode, setMode, trainingRank, setTrainingRank, selectedTier, busy, join, train, winStreak, bestWinStreak, clocks }) {
  ...
  return <section className="arena-game-shell" data-mode={mode} aria-label={`${exam} Battle Arena game lobby`}>
    ...
    <div className="arena-game-grid">
      ...
      <aside className="arena-match-console">
        ...
        <p className="arena-scale-copy">Every rank gets full-length, original {exam}-style questions. Higher ranks add denser passages, tighter traps, multi-constraint math, and dramatically harder reasoning.</p>
        ...
        {ranked && <p className="arena-match-note">{exam} opponents only. A bot joins after 30 seconds. One shared SAT + ACT rating.</p>}
      </aside>
    </div>
    <footer className="arena-game-ticker"><span>FIVE QUESTIONS. ONE SHARED CHALLENGE.</span><i /><span>ACCURACY WINS · SPEED BREAKS THE TIE</span><i /><span>GRANDMASTER = MAXIMUM {exam} DIFFICULTY</span></footer>
  </section>
}
...
export default function BattleArena() {
  ...
  const startBattle = nextBattle => { setExam(nextBattle.exam || 'SAT'); requestVersion.current += 1; setCalculatorOpen(false); setAnswers({}); setActiveQuestion(0); setBattle(nextBattle); if (nextBattle?.status === 'active') launchCinematic() }
  const join = async () => { startArenaMusic(); setBusy(true); setError(''); try { startBattle(await api('/api/sat-battles/queue', { method: 'POST', body: JSON.stringify({ exam }) })) } catch (x) { stopArenaMusic(); setError(x.message) } finally { setBusy(false) } }
  const train = async () => { startArenaMusic(); setBusy(true); setError(''); try { startBattle(await api('/api/sat-battles/train', { method: 'POST', body: JSON.stringify({ rank: trainingRank, exam }) })) } catch (x) { stopArenaMusic(); setError(x.message) } finally { setBusy(false) } }
  ...
  return <AppShell name={d.name}><main className={`app-main battle-page ${active ? 'battle-page--in-match' : waiting ? 'battle-page--queue' : complete ? 'battle-page--complete' : ''}`}>
    ...
    <section className="battle-lower"><div className="battle-rules"><small>HOW IT WORKS</small><h2>One clean round. No fluff.</h2><div><article><b>01</b><span><strong>Match</strong><p>Choose SAT or ACT. Match with a student preparing for the same exam, on the same question set.</p></span></article><article><b>02</b><span><strong>Race</strong><p>Answer all five before the clock runs out. It is sized to the tier, and it starts together.</p></span></article><article><b>03</b><span><strong>Climb</strong><p>Accuracy takes it. Faster completion breaks a tied score.</p></span></article></div></div><aside className="battle-leaderboard"><header><span><Trophy /> SAT + ACT LEADERBOARD</span><a href="#battle-rankings">View rankings</a></header>{d.leaderboard?.length ? d.leaderboard.slice(0, 5).map((row, index) => <div key={row.user_id}><i>{index + 1}</i><span>{String(row.user_name || 'M').slice(0, 1)}</span><b>{row.user_name}<small className={`battle-rank-label battle-rank-label--${row.rank.key}`}>{row.rank.label}</small></b><strong>{row.rating}</strong></div>) : <p>The first completed battle earns a place here.</p>}</aside></section>
