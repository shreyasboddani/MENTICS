...
function BattleArena() {
  ...
  return <AppShell name={d.name}><main className={`app-main battle-page ${active ? 'battle-page--in-match' : waiting ? 'battle-page--queue' : complete ? 'battle-page--complete' : ''}`}>
    {idle && <section className="arena-lobby" aria-label="SAT Battle Arena lobby"><header className="arena-lobby-topline"><span><i /> MENTICS SAT ARENA</span><b>SEASON 01</b><span>RANKED PLAY LIVE</span></header><div className="arena-lobby-main"><div className="arena-lobby-copy"><small>READY ROOM</small><h1>ENTER THE<br /><em>ARENA.</em></h1><p>Five original SAT questions. Two minutes. Accuracy decides the round; speed breaks the tie.</p><div className="arena-lobby-modes"><button className="arena-mode-card arena-mode-card--ranked" onClick={join} disabled={busy}><span><Swords /></span><div><small>RANKED DUEL</small><b>{busy ? 'Finding challenger…' : 'Find a match'}</b><em>Rating on the line</em></div><ArrowRight /></button><button className="arena-mode-card arena-mode-card--training" onClick={train} disabled={busy}><span><Brain /></span><div><small>TRAINING SIM</small><b>Challenge the bot</b><em>Zero rating impact</em></div><ArrowRight /></button></div></div><div className="arena-lobby-core" aria-hidden="true"><div><span className="arena-lobby-orbit arena-lobby-orbit--one" /><span className="arena-lobby-orbit arena-lobby-orbit--two" /><i><Swords /></i><b>1V1</b><small>DUEL READY</small></div></div><aside className={`arena-lobby-rank arena-lobby-rank--${rank?.key || 'bronze'}`}><small>YOUR RANK</small><div><span><Trophy /></span><h2>{rank?.label || 'Bronze'}<em>{rank?.rating || 1000} RP</em></h2></div><p>{rank?.nextAt ? `${Math.max(0, rank.nextAt - rank.rating)} RP to ${rank.nextLabel}` : 'You are at the top of the arena.'}</p><i><b style={{ width: `${rankProgress}%` }} /></i><dl><div><dt>WINS</dt><dd>{d.battleStats?.wins || 0}</dd></div><div><dt>ROUNDS</dt><dd>{d.battleStats?.battles_played || 0}</dd></div></dl></aside></div><footer className="arena-lobby-format"><span><i>01</i><b>RANKED</b><small>Match a student and climb RP.</small></span><span><i>05</i><b>QUESTIONS</b><small>Same SAT-style set for both players.</small></span><span><i>2:00</i><b>ROUND CLOCK</b><small>Fastest perfect finish wins the tie.</small></span></footer></section>}
