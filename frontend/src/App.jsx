...
function BattleArena() {
  ...
  return <AppShell name={d.name}><main className={`app-main battle-page ${active ? 'battle-page--in-match' : ''}`}>
    ...
    <section className={`battle-rank-strip battle-rank-strip--${rank?.key || 'bronze'}`}><div className="battle-rank-medal"><Trophy /></div><div><small>YOUR ARENA RANK</small><h2>{rank?.label || 'Bronze'} <span>{rank?.rating || 1000} RP</span></h2><p>{rank?.nextAt ? `${Math.max(0, rank.nextAt - rank.rating)} RP to ${rank.nextLabel}` : 'You are at the top of the arena.'}</p><i><b style={{ width: `${rankProgress}%` }} /></i></div><dl><div><dt>WINS</dt><dd>{d.battleStats?.wins || 0}</dd></div><div><dt>ROUNDS</dt><dd>{d.battleStats?.battles_played || 0}</dd></div></dl></section>
