...
export default function QuickPractice() {
  ...
  return <AppShell name={boot.data.name}><main className={`app-main quick-prep ${round ? 'prep-in-round' : ''}`}>
    ...
    {!round ? <>
      ...
      <div className="prep-session-bar"><span><b>{exam.toUpperCase()}</b> {labels[exam][subject]} · {mode === 'sprint' ? 'Speed round' : 'Strategy practice'}</span><button className="text-button" onClick={reset}>{finished ? 'Change practice' : 'End round'}</button></div>
      {finished ? <div className="prep-results"><Trophy className="prep-trophy" /><div className="eyebrow">ROUND COMPLETE</div><h2 ref={questionHeading} tabIndex={-1}>{correct === round.length ? 'Clean sweep.' : 'Every miss is a next move.'}</h2><p>{correct} of {round.length} correct{mode === 'sprint' ? ` in ${formatTime(seconds)}` : ''}. {missed.length ? 'Review the explanation, then try the missed questions again.' : 'Keep the momentum with another mix of strategies.'}</p><div className="prep-result-stats"><span><b>{points}</b>round points</span><span><b>{Math.round(correct / round.length * 100)}%</b>accuracy</span><span><b>{bestStreak}</b>best streak</span></div><div className="prep-result-actions"><button className="button button--primary" onClick={() => start()}>New round <ArrowRight /></button>{missed.length > 0 && <button className="button button--quiet" onClick={() => start(missed.map(answer => answer.question))}><RotateCcw /> Retry {missed.length} missed</button>}<button className="text-button" aria-expanded={review} onClick={() => setReview(!review)}>{review ? 'Hide' : 'Review'} answers</button></div>{review && <div className="prep-review">{answers.map((answer, i) => <article key={i}><small>{answer.correct ? 'CORRECT' : 'PRACTICE AGAIN'} · {answer.question.skill}</small><h3>{answer.question.prompt}</h3><p>Your answer: {answer.question.options[answer.selected]}</p><b>Correct answer: {answer.question.options[answer.question.answer]}</b><p>{answer.question.explanation}</p></article>)}</div>}</div> : <>
        <div className="prep-hud"><span><Target /> {index + 1} / {round.length}</span><span><Flame /> {streak} streak</span><span><Zap /> {points} pts</span>{mode === 'sprint' && <span><Clock3 /> {formatTime(seconds)} <small>/ 5:00 target</small></span>}</div>
        ...
        <div className="prep-question"><div className="eyebrow">{question.skill}</div><h2 ref={questionHeading} tabIndex={-1}>{question.prompt}</h2><div className="prep-answers" role="group" aria-label="Answer choices">{question.options.map((option, i) => <button type="button" key={i} disabled={revealed} aria-pressed={selected === i} onClick={() => setSelected(i)} className={`${selected === i ? 'is-selected' : ''} ${revealed && i === question.answer ? 'is-correct' : ''} ${revealed && selected === i && i !== question.answer ? 'is-wrong' : ''}`}><span>{'ABCD'[i]}</span>{option}{revealed && i === question.answer && <Check aria-label="Correct answer" />}</button>)}</div>
