...
function AdaptivePractice() {
	...
	useEffect(() => {
		let active = true;
		api(`/api/adaptive/profile?track=${track}`).then((data) => {
...
function PathPage() {
	...
	const loadTasks = async (regenerate = false) => {
		...
		setError("");
		try {
			const data = await api(`/api/tasks?category=${encodeURIComponent(category)}${isTest ? `&track=${activeTrack}` : ""}`, regenerate ? {
				method: "POST",
...
function CheckStep({ step, coachKind, feedback, selected, onSelect, onCheck, onContinue, busy, replay, isMath = false }) {
	...
	const getHint = async () => {
		setHintBusy(true);
		try {
			const data = await api("/api/activity-hint", {
				method: "POST",
				body: JSON.stringify({
					kind: coachKind,
					question_id: step.id
